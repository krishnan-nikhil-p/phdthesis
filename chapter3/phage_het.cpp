...
const int K_bac=2000; // deme size for bacteria
...
unsigned int N_gen = 5000; // Run time in generations
int samp_id=595;
float alpha = 0.01;
...
float calcHet(long arr [N_demes][2]){
    double cnt =  0 ;
    long double H = 0.0;
    long int deme_pop_a1;
    long int deme_pop_a2;
    float meanH;
...
int main (int argc, char * argv[]){
    ...
    string termination = "_demes" + strDeme.str() +"_" +strTime.str() +"_.txt";
    ...
    gsl_rng_set(r, samp_id); // Seed RNG
    ...
    int record_time_het =50;
    int record_time_pop=5000;
    int write_time_het =10000;
    int write_time_pop =50000;
    ...
    while( (avgH>0) || (Hext<2*tao) || (t<1.1*assign_gene_time) ){
        ...
        if (((V_deme[m][0]+V_deme[m][1] )>0)&& (Bempty>0)){
            ...
            //binomial_distribution<int> distribution_0( (V_deme[m][0] +V_deme[m][1]) , (alpha/K_bac)*Bempty  ); //sample <alpha Bi Bi> from binomial distrib.
            //pAux[0] = float(V_deme[m][0] / (V_deme[m][0]+V_deme[m][1]))  *(alpha/K_bac)*Bempty;
            //pAux[1]  = float(V_deme[m][1] / (V_deme[m][0]+V_deme[m][1]))  *(alpha/K_bac)*Bempty;
            //pAux[2] = 1 - (alpha/K_bac)*Bempty;
            //gsl_ran_multinomial(r, 3, (V_deme[m][0] +V_deme[m][1] ), pAux, aAux); // Get next generation

            pAux[0] = V_deme[m][0]   *(alpha/K_bac);
            pAux[1]  = V_deme[m][1]  *(alpha/K_bac);
            pAux[2] = 1.0 - pAux[0] - pAux[1]; 

            gsl_ran_multinomial(r, 3, Bempty, pAux, aAux); // Get next generation


            //int atot = (V_deme[m][0] +V_deme[m][1] )*(alpha/K_bac)*Bempty;
            //cout << V_deme[m][0] +V_deme[m][1] <<" "<< (alpha/K_bac)<<" "<<Bempty<< endl;
            //cout<< atot<<endl;
            //atot = min(atot,Bempty); //atot : total phage to be absorbed (must be less than or equal to  Bempty)
            //atot = min(atot, int(V_deme[m][0] +V_deme[m][1]) );
            ...
            int a0 = aAux[0]; //number of allele 0  to be absorbed
            //a0 = min(a0,int(V_deme[m][0]));
            int a1 = aAux[1]; //assign rest to allele 1
            //cout<< atot<<endl;
            //a1 = min(a1, int(V_deme[m][1]));
        ...
        for(int m=1;m<N_demes-2;m++){
            ...
            if (((V_deme[m][0]+V_deme[m][1] )>0)&& (Bempty>0)){
                ...
                //pAux[0] = (V_deme[m][0] / (V_deme[m][0]+V_deme[m][1]))  *(alpha/K_bac)*Bempty;
                //pAux[1] = (V_deme[m][1] / (V_deme[m][0]+V_deme[m][1]))  *(alpha/K_bac)*Bempty;
                //pAux[2] = 1 - (alpha/K_bac)*Bempty;
                ...
                int Babs = 0; //keep track of how many absorption events occur
                int n=0;  //keep track of how any bacteria have been iterated through 
                //cout<< "POPULATION BEFORE"  <<" "<< atot<<" "<<V_deme[m][0]<< " " << V_deme[m][1] << endl;
                ...
                while((Babs< a0 )&&(n<K_bac)){
                    if (B_deme[m][n] ==0){
                        ...
                        Babs+=1;
                        //if (t == 1047971){
                         //   cout<< m << " " <<V_deme[m][0]<< " "<<V_deme[m][1] << " "<< atot<< " "<< " "<< a1 <<a0<< " "<<endl;
                        //}
                    }
                    n+=1;
                }
                ...
                //cout<< "POPULATION AFTER"  << V_deme[m][0]<< " " << V_deme[m][1] << endl;
            }
        ...
        /*if((t%write_time_het ==0)&&(t>assign_gene_time)){
            fhet.open(folder+hetName);
        ...
        if((t%write_time_pop ==0)&&(t>assign_gene_time)){
            fpop.open(folder+velName);
    ...
    /*for (int m = 0; m < N_demes; m++){
        ...
        //cout <<m<<" "<< B_health <<" " <<B_inf<<" " <<B_lys <<endl; //save bacterial profiles
    }
    //fpop.open(folder+velName);
    //fhet.open(folder+hetName);*/
    for(int dt=0; dt <pop_hist.size();dt++){
